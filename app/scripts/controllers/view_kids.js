...
angular.module('yapp')
  .controller('ViewKidsCtrl', function($scope, $location,$http,$window,toastr) {
      ...
      $scope.subDates= function(lastSeen){
        ...
        $http.post($window.localStorage.getItem("base_url")+"/get_class_users?_id&name&username&password&total_score&level&reading_dates",{"class":user.class }).then(function(response){
